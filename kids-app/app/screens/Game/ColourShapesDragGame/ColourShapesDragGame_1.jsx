...
const holes = {
    ...
    hole2: require('../../../assets/S2.png'),
    hole3: require('../../../assets/S3.png'),
    holes4: require('../../../assets/S4.png'),
};
...
const ColourShapesDragGame_1 = ({ navigation }) => {
    ...
    const [incorrectRoundsPlayed, setIncorrectRoundsPlayed] = useState(0);
    ...
    /*(() => {
        // Log image positions whenever they change
    ...
    useEffect(() => {
        if (pressedNextButton && (correctRoundsPlayed !== 0 || incorrectRoundsPlayed !== 0)) {
            setTimeout(() => {
                ...
                navigation.navigate('ColourShapesDragGame_2', {
                    startTime: startTime,
    ...
    const getTime = () => {
        ...
        const hours = currentDate.getHours().toString().padStart(2, '0'); // Gets the current hour (0-23)
        const minutes = currentDate.getMinutes().toString().padStart(2, '0'); // Gets the current minutes (0-59)
        const seconds = currentDate.getSeconds().toString().padStart(2, '0'); // Gets the current seconds (0-59)

        const formattedTime = `${hours}:${minutes}:${seconds}`;
        setStartTime(formattedTime); // Output: "14:05:30" (for example)
    }
    ...
    const handleNextRound = () => {
        ...
        if (areImagesPlacedCorrectly) {
            ...
            setCorrectRoundsPlayed(correctRoundsPlayed + 1); // Increment correct rounds count
        } else {
            setFailurePopupVisible(true);
            setIncorrectRoundsPlayed(incorrectRoundsPlayed + 1); // Increment incorrect rounds count
        }
...
export default ColourShapesDragGame_1;
